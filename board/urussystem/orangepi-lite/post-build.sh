...
checkboard()
{
    RPIBOARD=$(echo $BOARD_NAME | grep -rni - -e "$1" | wc -l)
    if [ $RPIBOARD -gt 0 ] ; then
        return 1
    fi
    return 0
}
...
if [ -e ${TARGET_DIR}/etc/inittab ]; then
    ...
    sed -i '/GENERIC_SERIAL/a\
ttyS0::respawn:/sbin/getty -L ttyS0 115200 vt100 # HDMI console' ${TARGET_DIR}/etc/inittab
fi
...
cp -rf $BOARD_DIR/etc ${TARGET_DIR}/
cp -f $BOARD_DIR/forwardnat ${TARGET_DIR}/usr/sbin/
